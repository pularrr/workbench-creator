...
export class PostgresJobStore implements JobStore {
  ...
  async save(job: StoredAgentJob, text: string) {
    ...
    await this.db.execute(sql`insert into agent_jobs (id, session_id, node_id, kind, query, state, progress, error, result_object_key, result_checksum, result_bytes, revision, retry_count, worker_id, created_at, updated_at, started_at, finished_at, source_text, source_kind, code_repository_id)
      values (${job.id}::uuid, ${job.sessionId}, ${job.nodeId}, ${job.kind}, ${job.query}, ${job.state}, ${job.progress ?? null}, ${job.error ?? null}, ${objectKey}, ${checksum}, ${bytes.length}, ${job.revision}, ${job.retryCount}, ${job.workerId ?? null}, ${job.createdAt}, now(), ${job.startedAt ?? null}, ${job.finishedAt ?? null}, ${job.sourceText ?? null}, ${job.sourceKind ?? null}, ${job.codeRepositoryId ?? null})
      on conflict (id) do update set session_id=excluded.session_id, node_id=excluded.node_id, kind=excluded.kind, query=excluded.query, state=excluded.state, progress=excluded.progress, error=excluded.error, result_object_key=excluded.result_object_key, result_checksum=excluded.result_checksum, result_bytes=excluded.result_bytes, revision=excluded.revision, retry_count=excluded.retry_count, worker_id=excluded.worker_id, updated_at=now(), started_at=excluded.started_at, finished_at=excluded.finished_at, source_text=excluded.source_text, source_kind=excluded.source_kind, code_repository_id=excluded.code_repository_id`);
